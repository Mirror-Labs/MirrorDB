...
import { Attribute, EntityID, Fact, TripplePatternClause, Value, Variable } from "../Types.js";
...
function compileClause(clause: Clause): Types.Clause {
  const exprs: Types.Expr[] = clause.map(compileExpr);
  return new TripplePatternClause(exprs[0], clause[1], exprs[2]);
}
...
function compileExpr(expr: Expr): Types.Expr {
  ...
  if (typeof expr === 'string') {
    return expr.charAt(0) === '?' ? new Types.ExprVar(expr) : new Types.ExprVal(expr);
  }
...
export function exec(facts: Types.TrippleFact[], query: Query) {
  const result: Map<string, Value>[] = generateBindings(facts, query.where.map(compileClause));

  return result.map((m) => {
    const newM = new Map();
    for (const entry of m) {
      if (query.find.find((v) => v === entry[0])) {
        newM.set(entry[0], entry[1]);
      }
    }
    return newM;
  });
...
function generateBindings(facts: Fact[], clauses: Types.Clause[]): Map<string, Value>[] {
  const goals = clauses.map((c) => evalClause(facts, c));
  return goals.slice(1).reduce(unifyBindingsArrays, goals[0]);
}

function unifyBindingsArrays(prev: Map<string, Value>[], curr: Map<string, Value>[]): Map<string, Value>[] {
  const unifiedLists = prev.map(
      (prevBindings) => curr.map(
        (currBindings) => unifyBindings(prevBindings, currBindings))
...
function unifyBindings(bindings1: Map<string, Value>, bindings2: Map<string, Value>): Map<string, Value> | undefined {
...
function union(bindings1: Map<string, Value>, bindings2: Map<string, Value>): Map<string, Value> {
...
function unify(clause: Types.Clause, fact: Fact): Map<string, Value> | undefined {
  if (clause.isTripplePattern() && fact.isTripple()) {
    const result = new Map();
    if (clause.attr !== fact.attribute) {
      return undefined;
    }
    if (clause.entity.isVal()
      && clause.entity.value !== fact.entity) {
      return undefined;
    }
    if (clause.entity.isVar()) {
      result.set(clause.entity.var, fact.entity);
    }
    if (clause.value.isVal()
      && clause.value.value !== fact.value) {
      return undefined;
    }
